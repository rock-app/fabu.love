  ...
  return new Promise((resolve, reject) => {
    readStream.on("end", async () => {
      ...
      if (!app) {
        ...
        resolve({ app: app, version: version });
      }
  ...
  return new Promise((resolve, reject) => {
    parser.parse().then((result) => {
      ...
      try {
        ...
        const active =
          result.mobileProvision.Entitlements["beta-reports-active"];
...
async function extractIpaIcon(filename, guid, team) {
  ...
  var { stderr, stdout } = await exec(
    path.join(pnfdefryDir, exeName + " -s _tmp ", tmpOut)
  );
  if (stderr) {
    throw stderr;
  }
  ...
  return new Promise((resolve, reject) => {
    parser
      ...
      .then((result) => {
        ...
        var appName =
          result["application-label"] ||
          result["application-label-zh-CN"] ||
          result["application-label-es-US"] ||
          result["application-label-zh_CN"] ||
          result["application-label-es_US"] ||
          label ||
          "unknown";
        ...
        ext === ".xml"
          ? unzip.Open.file(filepath).then((directory) => {
              const getMaxSizeImagePath = compose(
                ...
                filter(
                  (entry) =>
                    entry.path.indexOf(dir) >= 0 &&
                    entry.path.indexOf(".png") >= 0
                ),
                get("files")
              );
        ...
        fs.createReadStream(filepath)
          ...
          .pipe(
            etl.map((entry) => {
              ...
              const isXml = entryPath.indexOf(".xml") >= 0;
              if (
                (!isXml && entryPath.indexOf(iconPath) != -1) ||
                (isXml && entry.path.indexOf(maxSizePath) != -1)
              ) {
                console.log(entry.path);
                entry.pipe(etl.toFile(tempOut));
                resolve({ success: true, fileName: realPath });
